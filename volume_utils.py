...
def fetch_volume_data(exchange, symbol, volume_list):
    ohlcv = exchange.fetch_ohlcv(symbol, timeframe='1m', limit=4)  # 1분 간격의 최근 4개 OHLCV 데이터 가져오기
    ...
    if current_volume > (1.5 * (volume_1_min_ago + volume_2_min_ago + volume_3_min_ago) / 3):  # 현재 거래량이 이전 3분 평균의 1.5배 초과시
        return True
